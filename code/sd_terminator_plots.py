...
#New June2
sTime=datetime.datetime(2013,8,21,1,0)
...
filename='SD_RTI_Terminator_'+radar+'_'+sTime.strftime('%Y%m%d_%H%M')+'-'+eTime.strftime('%Y%m%d_%H%M')+'.png'
...
#fig=plotRti(sTime, rad=radar, eTime=eTime, bmnum=beam, figure=fig, coords='geo',plotTerminator=True)
#fig = plt.figure(figsize=(14,8)) #Define a figure with a custom size.\n",
#fig=plotRti(sTime, rad=radar, eTime=eTime, bmnum=beam, coords='geo', retfig=True,params=['power'], plotTerminator=True)
fig=plotRti(sTime, rad=radar, eTime=eTime, bmnum=beam, coords='geo', retfig=True,params=['power'],xtick_size=18,ytick_size=18, plotTerminator=True)
