...
def plotRti(sTime,rad,eTime=None,bmnum=7,fileType='fitex',params=['velocity','power','width'], \
  ...
  **Args**:
    ...
    * **[ax]** (matplotlib.figure.add_suplot() ) axis object for plot. Right Now: ax MUST be passed in! Need to add what code must do if it is None.
...
def drawAxes(ax,times,rad,cpid,bmnum,nrang,frang,rsep,bottom,myFig=None,yrng=-1,coords='gate',pos=[.1,.05,.76,.72],xtick_size=9,ytick_size=9,xticks=None,axvlines=None):
  """draws empty axes for an rti plot

  **Args**:
    * **myFig**: the MPL figure we are plotting to
    * **ax**: the blank axis we are modifying 
