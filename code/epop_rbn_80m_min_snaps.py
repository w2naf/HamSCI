...
#ePOP RRI reciever frequency range (kHz)
f40l=7010
f40u=7040
f80l=3510
f80u=3540
...
#Restrict rbn recievers to 80m 
df2=rbn_df[rbn_df['band']=='80m']
df_epop=rbn_df[rbn_df['freq']<=f80u]
df_epop=df_epop[df_epop['freq']>=f80l]
...
while map_sTime<eTime:
    ...
    df_epop['Lower']=map_sTime
    df_epop['Upper']=map_eTime
#    df2['Lower']=map_sTime
#    df2['Upper']=map_eTime
    #import ipdb; ipdb.set_trace()
    #Clip according to the range of time for this itteration
    df_map=df_epop[(df_epop.Lower <= df_epop.date) & (df_epop.date < df_epop.Upper)]
...
#    rbn_df2  = rbn_lib.read_rbn(map_sTime,map_eTime,data_dir='data/rbn')
    ...
    good_loc    = df_map.dropna(subset=['dx_lat','dx_lon','de_lat','de_lon'])
    good_count_map  = good_loc['callsign'].count()
    total_count_map = len(df_map)
...
#    df2_temp=rbn_df2[rbn_df['band']=='40m']
    m,fig=rbn_lib.rbn_map_plot(df_map,legend=False,ax=ax0,tick_font_size=9,ncdxf=True, llcrnrlat=0,llcrnrlon=-135,urcrnrlon=45)
