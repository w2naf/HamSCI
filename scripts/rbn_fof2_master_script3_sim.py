...
def gen_map_run_list(spot_dir,integration_time,interval_time,**kw_args):
    files       = glob.glob(os.path.join(spot_dir,'*.csv'))
    dct_list    = []
    for fl in files:
        bn  = os.path.basename(fl)[:13]
        this_sTime      = datetime.datetime.strptime(bn,'%Y-%m-%d_%H')
...
class LocalLocator(object):
    def __init__(self,location_dir,qrz_call=None,qrz_passwd=None):
        try:
            self.qz = qrz.QRZSession(qrz_call,qrz_passwd)
        except:
            self.qz = None
            print('WARNING: No qrz.com connection.')

        files   = glob.glob(os.path.join(location_dir,'*.csv'))

        df_loc  = None
        for fl in files:
            df_tmp  = pd.read_csv(fl,header=0,names=['call','lat','lon'])

            if df_loc is None:
                df_loc  = df_tmp
            else:
                df_loc=pd.concat([df_loc,df_tmp],ignore_index=True,axis=0)
        df_loc  = df_loc.set_index('call')

        tf          = np.logical_and(df_loc.lat==-90.,df_loc.lon==-180.)
        bad_calls   = df_loc.index[tf]

        for call in bad_calls:
            result  = self.qrz_lookup(call)

            df_loc.loc[call]['lat'] = result[0]
            df_loc.loc[call]['lon'] = result[1]

        self.df_loc = df_loc
    
    def qrz_lookup(self,call):
        try:
            qz_obj  = self.qz.lookup_callsign(call)
            result  = (qz_obj['lat'],qz_obj['lon'])
        except:
            result  = (None,None)
    ...
    def locate(self,call):
        if call in self.df_loc.keys():
            row     = self.df_loc.loc[call]
...
def create_rbn_obj(sTime,eTime,
    ...
    if sim_file is not None:
        ...
        cols['srch_rd_apogee']                  = 'refl_height'
        cols['srch_rd_plasma_freq_at_apogee']   = 'refl_plasma_freq'
        cols['i']                               = 'hop_nr'
        cols['srch_rd_nhops_attempted.1']       = 'hops_attempted'
        ...
        for call in de_calls:
            lat,lon             = locator.locate(call)
            tf                  = df.callsign == call
            df.loc[tf,'de_lat'] = lat
            df.loc[tf,'de_lon'] = lon

        for call in dx_calls:
            lat,lon             = locator.locate(call)
            tf                  = df.dx == call
...
#    reflection_type     = 'sp_mid'
    reflection_type     = 'sim_no_eclipse'
...
#    interval_time           = datetime.timedelta(minutes=60)
    ...
    spot_dir                = os.path.join('data','sim_data','spots')

    location_dir            = os.path.join('data','sim_data','locations')
    locator                 = LocalLocator(location_dir,'w2naf','hamscience')
...
#    dct['call_filt_de'] = 'aa4vv'
    ...
    run_list            = gen_map_run_list(spot_dir,integration_time,interval_time,**dct)
